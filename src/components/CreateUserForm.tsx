...
export const CreateUserForm = ({ asPopup = false }: CreateUserFormProps) => {
	...
	const validationSchema = Yup.object({
		...
		role: Yup.string()
			.oneOf(
				['admin', 'sello', 'artista'],
				'Debes seleccionar un tipo de usuario válido'
			)
	...
	const formContent = (
		...
		>
			...
			{/* Role Field */}
			<div>
				<label className="text-gray-700 dark:text-gray-300" htmlFor="role">
					Tipo de Usuario
				</label>
				<select
					id="role"
			...
			{/* Picture URL Field */}
			<div>
				<label className="text-gray-700 dark:text-gray-300" htmlFor="picture">
					URL de la Imagen
				</label>
				<input
					type="text"
				...
				{formik.touched.picture && formik.errors.picture && (
					<p className="text-red-500 text-sm mt-1">{formik.errors.picture}</p>
				)}
			</div>
